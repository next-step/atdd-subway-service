...
class FavoriteServiceTest {
	...
	void testCreateFavorite() {
		FavoriteService service = new FavoriteService(stationService, memberService, favoriteRepository);
		Member member = new Member("email@email.com", "password", 20);
		Mockito.when(memberService.findById(Mockito.eq(1L)))
			.thenReturn(member);
		Station 강남역 = new Station("강남역");
		Mockito.when(stationService.findById(Mockito.eq(1L))).thenReturn(강남역);
		Station 광교역 = new Station("광교역");
		Mockito.when(stationService.findById(Mockito.eq(2L))).thenReturn(광교역);

		Favorite favorite = new Favorite(member, 강남역, 광교역);
		Mockito.when(favoriteRepository.save(Mockito.any())).thenReturn(favorite);
