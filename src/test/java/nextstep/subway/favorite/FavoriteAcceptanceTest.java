...
public class FavoriteAcceptanceTest extends FavoriteAcceptanceTestFixture {
    ...
    void findFavorites() {
        // Given
        FavoriteResponse 생성된_즐겨찾기 = 즐겨찾기_등록되어_있음(myAccessToken, new FavoriteCreateRequest(강남역.getId(), 정자역.getId()));
        FavoriteResponse 생성된_즐겨찾기2 = 즐겨찾기_등록되어_있음(myAccessToken, new FavoriteCreateRequest(광교역.getId(), 양재역.getId()));
    ...
    void deleteFavorite() {
        ...
        // When 즐겨찾기 삭제 요청
        ExtractableResponse<Response> response = 즐겨찾기_삭제_요청(myAccessToken, 생성된_즐겨찾기.getId());
        // Then 즐겨찾기 삭제됨
        즐겨찾기_삭제됨(response);
    }
}