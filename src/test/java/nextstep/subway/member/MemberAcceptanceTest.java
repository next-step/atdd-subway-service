...
public class MemberAcceptanceTest extends AcceptanceTest {
	...
	void manageMyInfo() {
		...
		String accessToken = AuthAcceptanceTest.로그인_요청(EMAIL, PASSWORD).jsonPath().getString("accessToken");

		ExtractableResponse<Response> response = 나의_정보_요청(accessToken);
		나의_정보_조회됨(response);

		ExtractableResponse<Response> updateResponse = 나의_정보_수정_요청(accessToken, new MemberRequest(NEW_EMAIL, NEW_PASSWORD, NEW_AGE));
		accessToken = AuthAcceptanceTest.로그인_요청(NEW_EMAIL, NEW_PASSWORD).jsonPath().getString("accessToken");
	...
	private ExtractableResponse<Response> 나의_정보_수정_요청(String accessToken, MemberRequest memberRequest) {
		return RestAssured.given().log().all()
				.auth()
				.oauth2(accessToken)
	...
	public static ExtractableResponse<Response> 나의_정보_요청(String accessToken) {
		return RestAssured.given().log().all()
				.auth()
				.oauth2(accessToken)
				.when()
