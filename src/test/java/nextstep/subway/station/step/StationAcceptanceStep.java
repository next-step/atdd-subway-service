...
public class StationAcceptanceStep {
    ...
    public static void 삭제한_지하철_역이_반영된_역_목록이_조회됨(ExtractableResponse<Response> response, List<StationResponse> stations) {
        List<Long> resultStationIds = response.jsonPath().getList(".", StationResponse.class).stream()
                .map(StationResponse::getId)
                .collect(Collectors.toList());

        List<Long> removeStationIds = stations.stream().map(StationResponse::getId).collect(Collectors.toList());
        resultStationIds.retainAll(removeStationIds);

        assertThat(resultStationIds.size()).isEqualTo(1);
        assertThat(response.statusCode()).isEqualTo(HttpStatus.OK.value());
    }
}
