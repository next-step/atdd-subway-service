...
class SectionAcceptanceTest extends AcceptanceTest {
    ...
    public void setUp() {
        ...
        선릉역 = StationAcceptance.create_station("선릉역").as(StationResponse.class);
        삼성역 = StationAcceptance.create_station("삼성역").as(StationResponse.class);
    }
    ...
    void addUnorderedSection() {
        ...
        SectionAcceptance.update_section(이호선.getId(), 선릉역.getId(), 삼성역.getId(), 10);
        SectionAcceptance.update_section(이호선.getId(), 교대역.getId(), 강남역.getId(), 10);

        // then
        LineResponse lineResponse = LineAcceptance.line_was_queried(이호선.getId());
        SectionAcceptance.stations_are_sorted_on_the_subway_line(lineResponse, Arrays.asList(교대역, 강남역, 역삼역, 선릉역, 삼성역));
    }
