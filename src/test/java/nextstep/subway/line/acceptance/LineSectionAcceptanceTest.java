...
public class LineSectionAcceptanceTest extends AcceptanceTest {
    ...
    void manageSection() {
        ...
        // when
        ExtractableResponse<Response> 조회_응답 = LineAcceptanceTest.지하철_노선_조회_요청(신분당선);

        // then
        지하철_노선에_지하철역_순서_정렬됨(조회_응답, Arrays.asList(강남역, 양재역, 광교역));
        ...
        // when
        ExtractableResponse<Response> 삭제후_조회_응답 = LineAcceptanceTest.지하철_노선_조회_요청(신분당선);

        // then
        지하철_노선에_지하철역_순서_정렬됨(삭제후_조회_응답, Arrays.asList(강남역, 광교역));
    }

    /**
     * When 지하철 노선에 이미 등록되어 있는 역을 등록
     * Then 지하철 구간 등록 실패
     * When 지하철 노선에 등록되지 않은 역을 등록
     * Then 지하철 구간 등록 실패
     * When 지하철 노선에 등록된 지하철역이 두개일 때 한 역을 삭제
     * Then 지하철 구간 삭제 실패
     */
    @DisplayName("구간을 관리하면서 생기는 예외상황")
    @Test
    void exceptions() {
        // when
        ExtractableResponse<Response> 중복_등록_응답 = 지하철_노선에_지하철역_등록_요청(신분당선, 강남역, 광교역, 3);

        // then
        지하철_노선에_지하철역_등록_실패됨(중복_등록_응답);

        // when
        ExtractableResponse<Response> 없는역_등록_응답 = 지하철_노선에_지하철역_등록_요청(신분당선, 정자역, 양재역, 3);

        // then
        지하철_노선에_지하철역_등록_실패됨(없는역_등록_응답);

        // when
        ExtractableResponse<Response> 마지막_삭제_응답 = 지하철_노선에_지하철역_제외_요청(신분당선, 강남역);

        // then
        지하철_노선에_지하철역_제외_실패됨(마지막_삭제_응답);
    }
