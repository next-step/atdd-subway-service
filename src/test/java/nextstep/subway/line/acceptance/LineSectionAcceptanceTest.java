...
import static nextstep.subway.line.acceptance.LineSectionTestFixture.지하철_노선에_지하철역_제외_실패됨;
import static nextstep.subway.line.acceptance.LineSectionTestFixture.지하철_노선에_지하철역_제외_요청;
import static nextstep.subway.line.acceptance.LineSectionTestFixture.지하철_노선에_지하철역_제외됨;
...
public class LineSectionAcceptanceTest extends AcceptanceTest {
    ...
    public void setUp() {
        ...
        신분당선 = LineTestFixture.지하철_노선_등록되어_있음(lineRequest).as(LineResponse.class);
    }
    ...
    @TestFactory
    Stream<DynamicTest> section_manage() {
        return Stream.of(
                DynamicTest.dynamicTest("지하철 구간을 등록한다.", () -> {
                    ExtractableResponse<Response> response = 지하철_노선에_지하철역_등록_요청(신분당선, 강남역, 양재역, 3);

                    응답이_성공임(response);
                }),
                DynamicTest.dynamicTest("지하철 노선에 등록된 역 목록 조회 요청", () -> {
                    ExtractableResponse<Response> response = LineTestFixture.지하철_노선_조회_요청(신분당선);

                    응답이_성공임(response);
                    지하철_노선에_지하철역_순서_정렬됨(response, Arrays.asList(강남역, 양재역, 광교역));
                }),
                DynamicTest.dynamicTest("지하철 구간 삭제를 요청한다.", () -> {
