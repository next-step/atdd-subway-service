...
public class LineSectionAcceptanceTest extends AcceptanceTest {
     ...
     */
    @TestFactory
    @DisplayName("지하철 구간 관련 기능 인수 테스트 통합")
    Stream<DynamicTest> lineSectionAcceptanceTest() {
        return Stream.of(
                DynamicTest.dynamicTest("지하철 구간 등록 요청하면 지하철 구간이 등록된다.", () -> {
                    지하철_노선에_지하철역_등록됨(지하철_노선에_지하철역_등록_요청(신분당선, 강남역, 양재역, 3));
                }),
                DynamicTest.dynamicTest("지하철 노선에 등록된 역 목록 조회 요청하면 등록한 지하철 구간이 반영된 역 목록이 조회된다", () -> {
                    지하철_노선에_지하철역_순서_정렬됨(지하철_노선_조회_요청(신분당선), Arrays.asList(강남역, 양재역, 광교역));
                }),
                DynamicTest.dynamicTest("지하철 구간 삭제 요청하면 지하철 구간 삭제된다.", () -> {
                    지하철_노선에_지하철역_제외됨(지하철_노선에_지하철역_제외_요청(신분당선, 양재역));
                }),
                DynamicTest.dynamicTest("지하철 노선에 등록된 역 목록 조회 요청하면 삭제한 지하철 구간이 반영된 역 목록이 조회된다.", () -> {
