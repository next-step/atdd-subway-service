...
public class LineAcceptanceTest extends AcceptanceTest {
    ...
    public void setUp() {
        ...
        lineRequest3 = new LineRequest("삼호선", "bg-red-600", 강남역.getId(), 광교역.getId(), 10, 3);
    }
    ...
    @DisplayName("추가요금을 포함하여 노선을 생성한다")
    @Test
    void line1() {
        // when
        ...
        // then
        지하철_노선_생성됨(response);
        ExtractableResponse<Response> 노선조회응답 = 지하철_노선_조회_요청(response.as(LineResponse.class));
        지하철_노선_응답됨(노선조회응답, lineRequest3.getAdditionalFee());
    }

    @DisplayName("추가요금을 포함하여 노선을 수정한다")
    @Test
    void line2() {
        // given
        ExtractableResponse<Response> createResponse = 지하철_노선_등록되어_있음(lineRequest1);
        ...
        // when
        ExtractableResponse<Response> response = 지하철_노선_수정_요청(createResponse, lineRequest3);

        // then
        지하철_노선_수정됨(response);
        ExtractableResponse<Response> 노선조회응답 = 지하철_노선_조회_요청(createResponse.as(LineResponse.class));
        지하철_노선_응답됨(노선조회응답, lineRequest3.getAdditionalFee());
    }
    ...
    public static void 지하철_노선_응답됨(ExtractableResponse<Response> response) {
        ...
        assertThat(response.as(LineResponse.class).getAdditionalFee()).isNotNull();
    }

    public static void 지하철_노선_응답됨(ExtractableResponse<Response> response, Integer additionalFee) {
        assertThat(response.statusCode()).isEqualTo(HttpStatus.OK.value());
        assertThat(response.as(LineResponse.class)).isNotNull();
        assertThat(response.as(LineResponse.class).getAdditionalFee()).isEqualTo(additionalFee);
    }
