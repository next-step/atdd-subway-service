...
class SectionTest {
    ...
    void 구간_수정시_수정하려는_거리값이_현재_구간의_거리값보다_크거나_같으면_수정할_수_없음(int current, int request) {
        assertThatThrownBy(() -> {
            Section section = new Section(line, upStation, downStation, current);
            section.divideSection(new Section(line, upStation, new Station("판교역"), request));
        }).isInstanceOf(IllegalArgumentException.class)
    ...
    void 구간_삭제시_거리_연장(int current, int request) {
        Section section = new Section(line, upStation, downStation, current);
        section.extendSection(new Section(line, downStation, upStation, request));
