...
public class LineTest {
	...
	void getStationsByOrder_success() {
		Line line = new Line(신분당선, BG_RED_600, StationTest.선릉역, StationTest.역삼역, 5);
		line.addSection(new Section(line, StationTest.역삼역, StationTest.강남역, 3));
		line.addSection(new Section(line, StationTest.삼성역, StationTest.선릉역, 3));

		List<Station> actual = line.getOrderedStations();

		assertThat(Arrays.asList(StationTest.삼성역, StationTest.선릉역, StationTest.역삼역, StationTest.강남역)).isEqualTo(actual);
	}
	...
	void addSection_success() {
		...
		actual.addSection(new Section(actual, StationTest.역삼역, StationTest.강남역, 3));
		Line expected = new Line(신분당선, BG_RED_600, StationTest.역삼역, StationTest.강남역, 3);
	...
	void addSection_success2() {
		Line actual = new Line(신분당선, BG_RED_600);
		actual.addSection(new Section(null, StationTest.역삼역, StationTest.강남역, 3));
		Line expected = new Line(신분당선, BG_RED_600, StationTest.역삼역, StationTest.강남역, 3);
