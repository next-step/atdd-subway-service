...
class PathTest {
    ...
    void instance() {
        assertThatNoException()
            .isThrownBy(() -> Path.of(Stations.from(Arrays.asList(station("강남역"), station("양재역"))),
                Distance.from(5)));
    }
    ...
    void instance_lessThanTwo_thrownIllegalArgumentException() {
        assertThatIllegalArgumentException()
            .isThrownBy(
                () -> Path.of(
                    Stations.from(Collections.singletonList(station("강남역"))), Distance.from(1)))
    ...
    private static Stream<Arguments> instance_nullArgument_thrownIllegalArgumentException() {
        return Stream.of(
            Arguments.of(Stations.from(
                Arrays.asList(station("강남역"), station("양재역"))), null
            ),
