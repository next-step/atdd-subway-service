...
class FareDistanceTest {
	...
	void createAgeFareTest(int age, int fare) {
		...
		FareUserAge fareAge = FareUserAge.findFareAge(age);

		// then
		assertThat(FareDistance.calculateDistance(distance, fareAge)).isEqualTo(new Money(fare));
	}
	...
	void createFareTest(int km, int fare) {
		...
		FareUserAge fareAge = FareUserAge.findFareAge(30);
