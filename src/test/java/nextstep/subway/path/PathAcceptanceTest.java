...
public class PathAcceptanceTest extends AcceptanceTest {
	...
	void 최단_경로_조회하기_출발역과_도착역이_같은_경우() {
		memberAcceptanceTest.회원_생성을_요청("taminging@kakao.com", "taminging", 15);
		토큰 = authAcceptanceTest.로그인("taminging@kakao.com", "taminging").as(TokenResponse.class);
		ExtractableResponse<Response> response = 로그인_후_최단_경로_조회_요청하기(강남역.getId(), 강남역.getId(), 토큰.getAccessToken());
	...
	void 최단_경로_조회하기_출발역과_도착역이_연결이_되어_있지_않은_경우() {
		...
		ExtractableResponse<Response> response = 로그인_후_최단_경로_조회_요청하기(강남역.getId(), 서울역.getId(), 토큰.getAccessToken());
		최단_경로_조회하기_실패(response);
	}
	...
	void 최단_경로_조회하기_존재하지_않은_출발역이나_도착역을_조회_할_경우() {
		memberAcceptanceTest.회원_생성을_요청("taminging@kakao.com", "taminging", 15);
		토큰 = authAcceptanceTest.로그인("taminging@kakao.com", "taminging").as(TokenResponse.class);
		ExtractableResponse<Response> response = 로그인_후_최단_경로_조회_요청하기(9L, 10L, 토큰.getAccessToken());
		최단_경로_조회하기_실패(response);
	}

	private ExtractableResponse<Response> 로그인_후_최단_경로_조회_요청하기(Long source, Long target, String accessToken) {
		return get("/paths?source=" + source + "&target=" + target, accessToken);
	}
