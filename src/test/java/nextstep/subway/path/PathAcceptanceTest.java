...
public class PathAcceptanceTest extends AcceptanceTest {
    ...
    public void findPath() {
        ...
        // then
        지하철역_최단_경로_응답됨(response);
        지하철역_최단_경로_일치함(response.as(PathResponse.class), 8,
            Arrays.asList(교대역.toStation(), 남부터미널역.toStation(), 양재역.toStation()));

    }

    public static ExtractableResponse<Response> 지하철역_최단_경로_조회_요청(Long sourceId, Long targetId) {
        return RestAssured
            .given().log().all()
    ...
    private void 지하철역_최단_경로_응답됨(ExtractableResponse<Response> response) {
        assertThat(response.statusCode()).isEqualTo(HttpStatus.OK.value());
    }

    private void 지하철역_최단_경로_일치함(PathResponse pathResponse, int expectedDistance, List<Station> expectedStations) {
