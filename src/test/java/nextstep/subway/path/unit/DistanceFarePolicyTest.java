...
public class DistanceFarePolicyTest {
	...
	void construct() {
		...
		assertThatIllegalArgumentException().isThrownBy(() -> new DistanceFarePolicy(distance)).withMessage("거리가 불명확하여 요금 측정이 불가합니다.");
	}
	...
	void fare(int distance, int fare) {
		...
		assertThat(distanceFarePolicy.fare()).isEqualTo(fare);
	}
}
