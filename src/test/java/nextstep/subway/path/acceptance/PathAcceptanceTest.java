...
public class PathAcceptanceTest extends AcceptanceTest {
    ...
    Stream<DynamicTest> 환승을_하여_최단거리를_검색한다() {
        ...
        LineResponse 사호선 = 지하철_노선_생성_요청_및_검증(new LineRequest("4호선", "보라색", 강남역.getId(), 쿄잉역.getId(), 20))
                .as(LineResponse.class);
        ...
        return Stream.of(
                ...
                dynamicTest("2호선에 쿄잉역을 추가한다", 지하철_노선에_지하철역_등록_요청_및_확인(이호선, 광교역, 쿄잉역, 1)),
                dynamicTest("2호선에 광교역, 쿄잉역 추가를 확인한다", 지하철_노선에_지하철역_순서_정렬됨(이호선, 강남역, 정자역, 광교역, 쿄잉역)),
                ...
                dynamicTest("4호선에 광교역을 추가한다", 지하철_노선에_지하철역_등록_요청_및_확인(사호선, 정자역, 광교역, 3)),
                dynamicTest("4호선에 양재역, 정자역, 쿄잉역 추가를 확인한다", 지하철_노선에_지하철역_순서_정렬됨(사호선, 강남역, 양재역, 정자역, 광교역, 쿄잉역)),
                dynamicTest("강남역과 광교역 최단거리를 확인한다", 지하철_최단거리_요청_및_확인(강남역, 쿄잉역, Arrays.asList(강남역, 광교역, 쿄잉역), 3))
        );
    }
