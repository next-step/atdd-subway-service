...
public class PathService {
	...
	public PathResponse findShortestPath(LoginMember loginMember, PathRequest request) {
		if (request.isSourceEqualToTarget()) {
			throw new IllegalArgumentException("출발역과 도착역이 같은 경우, 최단 경로를 조회할 수 없습니다.");
		}
		SubwayMap subwayMap = new SubwayMap(lineRepository.findAll(), new DijkstraPathFinder());
