...
public class StandardFare implements Fare {
	...
	public void calculateFare(List<Station> path, List<Section> sections, int distance) {
		...
		if (distance <= DEFAULT_FARE_MAX_DISTANCE) {
			this.fare = DEFAULT_FARE + additionalFare;
			return;
		}

		if (distance <= STEP1_FARE_MAX_DISTANCE) {
			this.fare = DEFAULT_FARE + additionalFare + calculateStep1Fare(
				  distance - DEFAULT_FARE_MAX_DISTANCE);
			return;
		}

		this.fare = DEFAULT_FARE + additionalFare + calculateStep1Fare(STEP1_RANGE_DISTANCE)
			  + calculateStep2Fare(
