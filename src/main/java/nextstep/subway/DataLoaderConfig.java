...
public class DataLoaderConfig implements CommandLineRunner {
    ...
    public void run(String... args) {
        ...
        Line 신분당선 = new Line("신분당선", "red lighten-1", 강남역, 양재역, 10,900);
        Line 이호선 = new Line("2호선", "green lighten-1", 교대역, 강남역, 10,300);
        Line 삼호선 = new Line("3호선", "orange darken-1", 교대역, 양재역, 10, 0);
        삼호선.addSection(교대역, 남부터미널역, 7);

        lineRepository.saveAll(Lists.newArrayList(신분당선, 이호선, 삼호선));

        memberRepository.save(new Member("probitanima11@gmail.com", "11", 10));
        memberRepository.save(new Member("1@1", "1234", 25));
    }
}