...
- [x] Scenario: 두 역의 최단 거리 경로를 조회
  - [x] Given 지하철역이 등록되어있음
  - And 지하철 노선이 등록되어있음 
  - And 지하철 노선에 지하철역이 등록되어있음
  - When 출발역에서 도착역까지의 최단 거리 경로 조회를 요청
  - Then 최단 거리 경로를 응답
  - [x] And 총 거리도 함께 응답함
  - [x] And ** 지하철 이용 요금도 함께 응답함 **
...
- [x] Feature: 즐겨찾기를 관리한다.
  ...
  - [x] Scenario: 즐겨찾기를 관리
    ...
    - When 다른 회원으로 로그인 요청
    - Then 로그인이 성공됨
    - When 타 회원의 즐겨찾기를 삭제 요청
    - Then 타 회원의 즐겨찾기 삭제가 실패됨
...
### 피드백 반영 필요 목록
- [x] 즐겨찾기 관리 인수 테스트에 ***다른 회원의 즐겨찾기는 삭제할수 없다.*** 추가
- [x] 즐겨찾기 추가시 출발역과 도착역이 같은지 검증 로직 추가
- [x] 회원 서비스에 RuntimeException::new을 대신해서 적절한 커스텀 예외 처리 추가
- [x] 회원 서비스에 트랜잭션 어노태이션 추가를 통해 정상적으로 비즈니스 로직이 처리되도록 수정 
