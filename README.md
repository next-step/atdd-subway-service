...
## 4단계 - 요금 조회
- [ ] 노선별 추가 요금 정책 추가
  - [ ] Line에 `추가 요금` column 추가
    - [ ] 도메인 테스트 수정 및 추가
    - [ ] 인수 테스트 수정 및 추가
- [ ] 경로 조회 시 거리 기준 요금 정보 포함하기
  - [ ] 운임료 계산하는 역할을 담당하는 객체를 분리하면? --> 운임료 계산기(ChargeCalculator)
    (총 거리, 경로 중 가장 비싼 추가요금, (option 로그인 되어 있다면) 로그인 멤버의 연령)
    - 기본운임 (10km 이내 (excluded)) : 1,250원
    - 이용 거리 초과 시 추가 운임 부과
      - 10km 이상 ~ 50km 이내(10km Included ~ 50km excluded) : 5km 마다 100원
      - 50km 이상(Included) : 8km 마다 100원
    - [ ] `Sections` 의 경로 조회 기능의 인수 테스트 작성
    - [ ] `Sections` 의 경로조회 운임 확인에 대한 도메인 테스트 추가
      - 노선의 추가 요금을 고려한 운임 부과
      - 10 이내
      - 10 ~ 50 
      - 50 이상
- [ ] 연령별 할인 정책 추가
  - 로그인 시에만 연령별 할인 정책 추가
  - 비 로그인 시에는 할인 정책을 고려 하지 않음
  ``````