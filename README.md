...
### 4단계 각 요금 부과 정책에 관하여 리팩터링 가능성
이미 `정책` 이라는 단어에서 부터 요구사항이 빈번하게 바뀔것이 예상됨..  
따라서, 각 부과하는 메서드를 정책 클래스로 역할과 책임을 분산하면 어떨까?...

- [ ] 거리 구간별 요금 부과 **_정책_** --> bean 주입 **가능**(런타임 시 입력값 필요 없음)
- [ ] 구간별 요금 부과 **_정책_** --> bean 주입 **가능**(런타임 시 동적인 입력값 필요 없음)
- [ ] 연령별 요금 부과 **_정책_** --> bean 주입 **불가능**(정책 인스턴스 생성 시 요청자의 나이가 필요함)