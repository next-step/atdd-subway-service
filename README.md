...
1. 토큰 발급 기능 
    * 인수테스트 기반 기능 구현
        > - 이메일+패스워드 > access token
    * 단위테스트
        > - 유효하지 않은 토큰에 대해 예외 발생
2. 내정보  조회 기능
    * 인증 기반
        > - RestAssured .auth().oauth2(accessToken) 활용
...
1. 최단 경로 조회 인수테스트 작성
2. 최단 경로 조회 기능 구현
    * TDD의 방향보다는 테스트를 통한 기능 명세 및 리팩토링에 집중.
    * outside in (서비스.. mock객체..)
    * inside out (도메인..)    
    * 외부 라이브러리: 단위테스트 x, 실제 객체 이용 o
3. 단위테스트
    * TDD
    * 예외사항 검증
---
...
![image.png](https://nextstep-storage.s3.ap-northeast-2.amazonaws.com/127958fee3fd4e7fb0b2da0f8339ae2d)
